...
export class Lmenu extends Component {
  constructor(props, context) {
    ...
    const openKeys = Config.localItem("OPENKEY")
      ? [Config.localItem("OPENKEY")]
      : [];
    this.state = {
      openKeys: openKeys
    };
  ...
  ifShowUserManagement() {
    if (this.role === "tester" || this.role === "auditor") {
      return false;
    }
    return true;
  }
    ...
    const defaultSelectedKey =
      process.env.NODE_ENV !== "production"
        ...
        : [
            location.hash
              .split("/")
              [location.hash.split("/").length - 1].split("?")[0] || "home"
          ];
    return (
      <Menu
